...
import { Bot, GraduationCap, LayoutDashboard, FileText, Printer, LogOut, Loader2, Target, AlertTriangle, CheckCircle, Info, Sparkles, BookOpen, Link as LinkIcon, Download, Play, Wrench } from 'lucide-react';
import { generateAcademicPlan } from '../services/roadmapService';
...
export default function StudentPortal({ onLogout }: StudentPortalProps) {
  ...
  const [aiResources, setAiResources] = useState<ResourceItem[] | null>(null);
  ...
  const handleAnalyze = async () => {
    ...
    const { prevCgpa = 0, attend = 0, assign = 0, backlogs = 0, sem = 1, study = 0, screen = 0, sleep = 0, participation = 2, stress = 5 } = profile;
    const safePrevCgpa = isNaN(prevCgpa as any) ? 0 : prevCgpa;
    const safeAttend = isNaN(attend as any) ? 0 : attend;
    const safeAssign = isNaN(assign as any) ? 0 : assign;
    const safeBacklogs = isNaN(backlogs as any) ? 0 : backlogs;
    const safeSem = isNaN(sem as any) ? 1 : sem;
    const safeStudy = isNaN(study as any) ? 0 : study;
    const safeScreen = isNaN(screen as any) ? 0 : screen;
    const safeSleep = isNaN(sleep as any) ? 0 : sleep;
    const safeStress = isNaN(stress as any) ? 5 : stress;

    const x = [
      Math.min(safePrevCgpa, 10) / 10,
      Math.min(safeAttend, 100) / 100,
      Math.min(safeAssign, 100) / 100,
      Math.min(safeBacklogs, 10) / 10,
      Math.min(safeSem, 8) / 8,
      Math.min(safeStudy, 12) / 12,
      Math.min(safeScreen, 12) / 12,
      Math.min(safeSleep, 10) / 10,
      (participation || 2) / 3,
      safeStress / 10,
    ];
    ...
    const riskRaw = computeRisk(safePrevCgpa, safeAttend, safeAssign, safeBacklogs, safeSem, safeStudy, safeScreen, safeSleep, safeStress);
    const risk = isNaN(riskRaw) ? 0 : riskRaw;
    const confidenceValue = Math.max(...nnP);
    const confidence = isNaN(confidenceValue) ? '0.0' : (confidenceValue * 100).toFixed(1);

    setResult({ nnP, rfP, lrP, ensP, catIdx, cat, risk, confidence });
    setIsAnalyzing(false);
    
    // Generate AI Academic Plan (Roadmap + Resources) using Gemini
    setIsGeneratingRoadmap(true);
    const mlInsights = `Ensemble confidence: ${confidence}%. Major risk factors identified: ${safeBacklogs > 0 ? 'Active backlogs' : 'None'}. Suggesting ${cat} standing intervention.`;
    const plan = await generateAcademicPlan(profile, cat, risk, mlInsights);
    
    if (plan.roadmap && plan.roadmap.length > 0) {
      setAiRoadmap(plan.roadmap);
    }
    if (plan.resources && plan.resources.length > 0) {
      setAiResources(plan.resources);
    }
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="field">
                ...
                <input type="number" value={isNaN(profile.prevCgpa) ? '' : profile.prevCgpa} onChange={e => setProfile({ ...profile, prevCgpa: e.target.value === '' ? NaN : parseFloat(e.target.value) })} min="0" max="10" step="0.1" />
              </div>
              <div className="field">
                <label>Previous SGPA <span className="text-[10.5px] font-normal text-[var(--text3)] ml-1">(0–10)</span></label>
                <input type="number" value={isNaN(profile.prevSgpa) ? '' : profile.prevSgpa} onChange={e => setProfile({ ...profile, prevSgpa: e.target.value === '' ? NaN : parseFloat(e.target.value) })} min="0" max="10" step="0.1" />
              </div>
              <div className="field">
                <label>Attendance <span className="text-[10.5px] font-normal text-[var(--text3)] ml-1">(%)</span></label>
                <input type="number" value={isNaN(profile.attend) ? '' : profile.attend} onChange={e => setProfile({ ...profile, attend: e.target.value === '' ? NaN : parseFloat(e.target.value) })} min="0" max="100" />
              </div>
              <div className="field">
                <label>Assignment Score <span className="text-[10.5px] font-normal text-[var(--text3)] ml-1">(%)</span></label>
                <input type="number" value={isNaN(profile.assign) ? '' : profile.assign} onChange={e => setProfile({ ...profile, assign: e.target.value === '' ? NaN : parseFloat(e.target.value) })} min="0" max="100" />
              </div>
              <div className="field">
                <label>Active Backlogs</label>
                <input type="number" value={isNaN(profile.backlogs) ? '' : profile.backlogs} onChange={e => setProfile({ ...profile, backlogs: e.target.value === '' ? NaN : parseInt(e.target.value) })} min="0" max="20" />
              </div>
              <div className="field">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="field">
                ...
                <input type="number" value={isNaN(profile.study) ? '' : profile.study} onChange={e => setProfile({ ...profile, study: e.target.value === '' ? NaN : parseFloat(e.target.value) })} min="0" max="16" step="0.5" />
              </div>
              <div className="field">
              ...
              <div className="field">
                ...
                <input type="number" value={isNaN(profile.screen) ? '' : profile.screen} onChange={e => setProfile({ ...profile, screen: e.target.value === '' ? NaN : parseFloat(e.target.value) })} min="0" max="16" step="0.5" />
              </div>
              <div className="field">
                <label>Sleep Hours / day</label>
                <input type="number" value={isNaN(profile.sleep) ? '' : profile.sleep} onChange={e => setProfile({ ...profile, sleep: e.target.value === '' ? NaN : parseFloat(e.target.value) })} min="0" max="16" step="0.5" />
              </div>
              <div className="field">
                <label>Stress Level <span className="text-[10.5px] font-normal text-[var(--text3)] ml-1">(1–10)</span></label>
                <input type="number" value={isNaN(profile.stress as any) || profile.stress === undefined ? '' : profile.stress} onChange={e => setProfile({ ...profile, stress: e.target.value === '' ? NaN : parseInt(e.target.value) })} min="1" max="10" />
              </div>
            </div>
        ...
        {result && (
          <div ref={resultRef} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 mb-6">
              {[
                { name: 'Prev CGPA', val: isNaN(profile.prevCgpa as any) ? '0.00' : profile.prevCgpa.toFixed(2), pct: isNaN(profile.prevCgpa as any) ? 0 : profile.prevCgpa * 10 },
                { name: 'Prev SGPA', val: isNaN(profile.prevSgpa as any) ? '0.00' : profile.prevSgpa.toFixed(2), pct: isNaN(profile.prevSgpa as any) ? 0 : profile.prevSgpa * 10 },
                { name: 'Attendance', val: (isNaN(profile.attend as any) ? 0 : Math.round(profile.attend)) + '%', pct: isNaN(profile.attend as any) ? 0 : profile.attend },
                { name: 'Assignments', val: (isNaN(profile.assign as any) ? 0 : Math.round(profile.assign)) + '%', pct: isNaN(profile.assign as any) ? 0 : profile.assign },
                { name: 'Backlogs', val: isNaN(profile.backlogs as any) ? '0' : profile.backlogs.toString(), pct: isNaN(profile.backlogs as any) ? 0 : Math.min(profile.backlogs * 10, 100) },
                { name: 'Study Hrs', val: (isNaN(profile.study as any) ? 0 : profile.study).toFixed(1) + 'h', pct: isNaN(profile.study as any) ? 0 : (profile.study / 8) * 100 }
              ].map(m => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
              <div className="card">
                ...
                <div className="space-y-3">
                  {getFactors(isNaN(profile.prevCgpa as any) ? 0 : profile.prevCgpa, isNaN(profile.attend as any) ? 0 : profile.attend, isNaN(profile.assign as any) ? 0 : profile.assign, isNaN(profile.backlogs as any) ? 0 : profile.backlogs, isNaN(profile.study as any) ? 0 : profile.study, isNaN(profile.screen as any) ? 0 : profile.screen, isNaN(profile.sleep as any) ? 0 : profile.sleep).map((f, i) => (
              ...
              <div className="card">
                ...
                <div className="space-y-3 mb-6">
                  {[
                    { label: 'CGPA (×10)', val: isNaN(profile.prevCgpa as any) ? 0 : Math.round(profile.prevCgpa * 10) },
                    { label: 'Attendance', val: isNaN(profile.attend as any) ? 0 : Math.round(profile.attend) },
                    { label: 'Assignments', val: isNaN(profile.assign as any) ? 0 : Math.round(profile.assign) },
                    { label: 'Study Score', val: isNaN(profile.study as any) ? 0 : Math.min(Math.round((profile.study / 8) * 100), 100) },
                    { label: 'Sleep Score', val: isNaN(profile.sleep as any) ? 0 : Math.min(Math.round((profile.sleep / 9) * 100), 100) },
                    { label: 'Risk Score', val: isNaN(result.risk) ? 0 : result.risk }
                  ].map(b => (
            ...
            <div className="card mb-6">
              <div className="flex items-center justify-between mb-4">
                <div className="font-mono text-[9.5px] tracking-widest uppercase text-[var(--text3)]">Personalized Roadmap</div>
                {aiRoadmap && (
                  <div className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-[var(--purple-bg)] border border-[var(--purple-border)] text-[var(--purple)] text-[9px] font-bold uppercase tracking-tight">
              ...
              {activeTab === 'roadmap' ? (
                isGeneratingRoadmap ? (
                  ...
                  <div className="roadmap">
                    {(aiRoadmap || getRoadmap(result.cat, profile.prevCgpa, profile.attend, profile.backlogs, profile.study)).map((r, i) => (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {isGeneratingRoadmap ? (
                    <div className="col-span-full py-12 flex flex-col items-center justify-center gap-4 text-[var(--text3)]">
                      <Loader2 className="w-8 h-8 animate-spin text-[var(--purple)]" />
                      <div className="text-[13px] font-medium animate-pulse">Curating personalized learning materials...</div>
                    </div>
                  ) : aiResources ? (
                    aiResources.map((res, i) => (
                      <div key={i} className="bg-[var(--surface2)] border border-[var(--border)] rounded-xl p-4 flex gap-4 items-start hover:border-[var(--border2)] transition-colors group">
                        <div className="w-10 h-10 rounded-lg bg-[var(--bg3)] flex items-center justify-center shrink-0 text-[var(--text3)] group-hover:text-[var(--red)] transition-colors">
                          {res.type === 'course' && <GraduationCap className="w-5 h-5" />}
                          {res.type === 'book' && <BookOpen className="w-5 h-5" />}
                          {res.type === 'tool' && <Wrench className="w-5 h-5" />}
                          {res.type === 'video' && <Play className="w-5 h-5" />}
                          {res.type === 'article' && <FileText className="w-5 h-5" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="font-semibold text-[13.5px] text-[var(--text)] mb-1 truncate">{res.title}</div>
                          <div className="text-[11.5px] text-[var(--text3)] leading-relaxed mb-3 line-clamp-2">{res.desc}</div>
                          <div className="flex items-center justify-between">
                            <span className="text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-[var(--bg3)] text-[var(--text4)] border border-[var(--border)]">{res.type}</span>
                            <a href={res.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[11px] font-medium text-[var(--red)] hover:underline">
                              Access <LinkIcon className="w-2.5 h-2.5" />
                            </a>
                          </div>
                        </div>
                      </div>
