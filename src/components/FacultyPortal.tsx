    ...
    async function init() {
      ...
      const nn = await buildAndTrainNN(X, Y, (ep, total, logs) => {
        setTrainingStatus({
          ...
          log: `Epoch ${ep}/${total} — loss: ${logs.loss.toFixed(4)}`
        });
      });
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="field">
                ...
                <input type="number" value={isNaN(profile.prevCgpa as any) ? '' : profile.prevCgpa} onChange={e => setProfile({ ...profile, prevCgpa: e.target.value === '' ? NaN : parseFloat(e.target.value) })} min="0" max="10" step="0.1" />
              </div>
              <div className="field">
                <label>Previous SGPA</label>
                <input type="number" value={isNaN(profile.prevSgpa as any) ? '' : profile.prevSgpa} onChange={e => setProfile({ ...profile, prevSgpa: e.target.value === '' ? NaN : parseFloat(e.target.value) })} min="0" max="10" step="0.1" />
              </div>
              <div className="field">
                <label>Attendance (%)</label>
                <input type="number" value={isNaN(profile.attend as any) ? '' : profile.attend} onChange={e => setProfile({ ...profile, attend: e.target.value === '' ? NaN : parseFloat(e.target.value) })} min="0" max="100" />
              </div>
              <div className="field">
                <label>Assignment Score (%)</label>
                <input type="number" value={isNaN(profile.assign as any) ? '' : profile.assign} onChange={e => setProfile({ ...profile, assign: e.target.value === '' ? NaN : parseFloat(e.target.value) })} min="0" max="100" />
              </div>
              <div className="field">
                <label>Active Backlogs</label>
                <input type="number" value={isNaN(profile.backlogs as any) ? '' : profile.backlogs} onChange={e => setProfile({ ...profile, backlogs: e.target.value === '' ? NaN : parseInt(e.target.value) })} min="0" max="20" />
              </div>
              <div className="field">
            ...
            <div className={`class-banner p-6 rounded-[var(--radius)] border mb-6 grid grid-cols-1 sm:grid-cols-[auto_1fr_auto_auto] gap-5 items-center ${
              ...
              <div className="hidden sm:block text-right">
                <div className={`font-serif text-3xl font-semibold ${
                  ...
                  result.cat === 'avg' ? 'text-[#d07030]' :
                  'text-[#3db56e]'
                }`}>{isNaN(result.risk) ? 0 : result.risk}</div>
